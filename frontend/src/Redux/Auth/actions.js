...
export const loginUser = (loginDetails) => (dispatch) => {
  ...
  const config = {
    method: "post",
    url: "http://localhost:5000/asos/user/profile",
    headers: {
...
export const getActiveUser = () => (dispatch) => {
  ...
  if( data != null ) {
    ...
    const config = {
      method: "get",
      url: "http://localhost:5000/asos/users",
      headers: {
...
export const getCartData = () => (dispatch) => {
  ...
  if( data !== null ) {
    ...
    const config = {
      method: "get",
      url: "http://localhost:5000/asos/cart",
      headers: {
        "Authorization": `Bearer ${data.token}`
      },
    }
...
export const getOrderData = () => (dispatch) => {
  ...
  if( data !== null ) {
    ...
    const config = {
      method: "get",
      url: "http://localhost:5000/asos/order-details",
      headers: {
        "Authorization": `Bearer ${data.token}`
      },
    }
